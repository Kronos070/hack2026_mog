...
На панели управления игрок может включить переключатель **«Автовывод x2»** ([ActionBar.tsx](file:///Users/kenny/Work/hack2026_mog/frontend/src/features/bet/ActionBar.tsx)).
- При активном автовыводе фронтенд-контроллер полета ([use-round-controller.ts](file:///Users/kenny/Work/hack2026_mog/frontend/src/features/flight/use-round-controller.ts)) в реальном времени отслеживает каждый 60 FPS тик множителя в колбэке `handleTick`.
- Как только множитель достигает значения $\ge 2.00\times$ (и пройден 1-й высотный уровень разблокировки), клиент автоматически отправляет запрос `POST /api/game/cashout`.
- Это полностью исключает влияние человеческой задержки реакции (пинг, скорость клика игрока) при игре по популярной стратегии удвоения.
