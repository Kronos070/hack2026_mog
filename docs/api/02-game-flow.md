...
## 1. Запуск раунда (Взлет)

### `POST /api/game/start`
Инициализирует новый раунд, проверяет баланс, списывает ставку и начинает полет.
...
| `boosterMultiplier` | number | Нет | `1` | Множитель бустера: `1` (без бустера), `2` ($\times 2$), `3` ($\times 3$), `4` ($\times 4$). Поддерживается алиас `boosterTier`. |

#### Что происходит на сервере:
1. Если у игрока уже есть незавершенный активный раунд, сервер автоматически завершает его крашем.
2. Проверяется условие `bonusBalance >= betAmount`. При нехватке средств возвращается ошибка `400`.
3. Сумма ставки атомарно списывается с баланса игрока.
4. Сервер рассчитывает детерминированную точку краха с использованием криптографического HMAC-SHA256 (Provably Fair) и House Edge игрока.
5. Если выбран бустер (`boosterMultiplier > 1`), сервер случайно и честно назначает уровень его появления (от 2 до `totalLevels - 2`).
6. В базу данных сохраняется раунд со статусом `IN_PROGRESS`.
7. В виртуальном потоке запускается 60 FPS стрим тиков в открытый WebSocket игрока.
...
## 2. Полет шара и отслеживание множителя
...
## 3. Фиксация выигрыша (Cashout)

### `POST /api/game/cashout`
Игрок нажимает кнопку «Забрать». Сервер по текущей серверной метке времени фиксирует множитель и начисляет выигрыш.
...
6. **Мета-награды:** Сервер проверяет выпадение кусочка пазла по Pity Timer и открывает разблокированные достижения.
