...
3. [Механика выбора ставок и применения бустеров](#-3-механика-выбора-ставок-и-применения-бустеров)
   - 3.1. [Фрагменты пазла как ставки](#31-фрагменты-пазла-как-ставки)
...
5. [Краткое резюме мета-систем и удержания](#-5-краткое-резюме-мета-систем-и-удержания)
   - 5.1. [Коллекция из 9 пазлов и Pity Timer](#51-коллекция-из-9-пазлов-и-pity-timer)
...
### 3.1. Фрагменты пазла как ставки
На экране подготовки к полету игроку предлагаются **4 карточки фрагментов пазла**. Каждая карточка задает:
- Сумму ставки в бонусах (например, 50, 100, 250, 500);
- Множитель бустера:
  - **Фрагмент 1:** Без бустера ($\times 1$)
  - **Фрагмент 2:** Бустер $\times 2$
  - **Фрагмент 3:** Бустер $\times 3$
  - **Фрагмент 4:** Бустер $\times 4$

Если баланс пользователя меньше стоимости фрагмента, кнопка выбора блокируется с сообщением *«Не хватает бонусов»*.
...
### 5.1. Коллекция из 9 пазлов и Pity Timer
- В игре есть коллекционный пазл из **9 уникальных фрагментов** (`piece_1` ... `piece_9`);
- По завершении каждого раунда рассчитывается шанс выпадения недостающего фрагмента с алгоритмом **Bad Luck Protection (Pity Timer)**:
  ...
  - Защита от повторов: выпадают только еще не собранные фрагменты.
