-- Migration V1.0.12: Seed test user "Тестовый Пользователь"
-- Username: test_user
-- Password: test1234
-- Bonus Balance: 10 000 bonuses
-- Rank: Average rank ("aeronaut" / "Воздухоплаватель" in meta-game, mid-tier in tournament)
-- Achievements: None

DO $$
DECLARE
    u_id BIGINT;
    -- BCrypt hash for password "test1234" (standard 10 rounds, verified with Elytron BcryptUtil)
...
BEGIN
    ...
    -- 4. Tournament entry with score = 500 (approx. rank 18 of 36 in tournament leaderboard)
    ...
    -- Rank "aeronaut" ("Воздухоплаватель", rank 3 of 6, minProfit: 2000, nextAt: 5000)
    -- Target profit: 3500 (totalPayout: 6300, totalWagered: 2800) -> 50% progress in middle rank
    -- All bets = 200 (< 250, so no high_roller)
    -- All multipliers <= 4.50 (< 5.0, so no high_flight / stratosphere / risky_captain)
    -- Alternating win/loss (max streak = 1, so no win_streak_3)
    -- booster_activated = false (no booster_master)
