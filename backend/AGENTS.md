...
9. **Сохранение обратной совместимости API-контрактов:**
   При реализации новых фич критически важно **не ломать старые контракты API** (структуру DTO, REST-эндпоинты, форматы WebSocket-сообщений). В случае возникновения необходимости сломать контракт, **обязательно предварительно задать вопрос пользователю** и согласовать дальнейшие действия, прежде чем вносить изменения.
...
2. **В процессе работы:**
   ...
   - **Не ломай старые контракты API:** строго сохраняй обратную совместимость существующих контрактов; если для реализации новой фичи необходимо изменить контракт — обязательно спроси пользователя, как поступить.
