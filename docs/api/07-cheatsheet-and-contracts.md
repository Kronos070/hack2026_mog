...
Этот раздел — практическая настольная шпаргалка для фронтенд-разработчиков и тестировщиков: все типы данных на TypeScript, сводная таблица всех 24 эндпоинтов, расшифровка ошибок и чек-лист для проверки интеграции.
...
|---|---|---|---|
| **`200`** | OK | Успешный запрос (вход, кэшаут, профиль, турнир). | Отобразить данные в UI. |
...
| **`400`** | Bad Request | Ошибка валидации; недостаточно бонусов на ставку; попытка нажать «Забрать» до 1-го уровня. | Показать всплывающий тост с текстом из `message`. Не блокировать UI. |
...
| **`403`** | Forbidden | Пользователь с ролью `USER` пытается вызвать админку (`PUT /api/admin/config`). | Показать сообщение «Доступ запрещен (требуется роль ADMIN)». |
...
export interface RewardDto {
  kind: "puzzle-piece";
  pieceId: string; // "piece_1" ... "piece_9" или "completed"
  label: string;
  collected: number;
  total: number; // 9
}
...
export interface ProfileDto {
  user: ProfileUserDto;
  puzzle: string[]; // собранные фрагменты ["piece_1", "piece_3"]
...
| `POST` | `/api/auth/register` | Нет | [01. Аутентификация](./01-auth-and-users.md) | Регистрация (+1000 бонусов) |
...
| `POST` | `/api/users/me/top-up` | JWT | [01. Аутентификация](./01-auth-and-users.md) | Тестовое пополнение своего баланса |
| `POST` | `/api/users/{id}/top-up` | Нет | [01. Аутентификация](./01-auth-and-users.md) | Тестовое пополнение любого игрока |
| `POST` | `/api/game/start` | JWT | [02. Игровой цикл](./02-game-flow.md) | Старт раунда полета (списание ставки) |
...
Перед показом проекта убедитесь, что пройдены все 10 шагов:

1. [ ] **Регистрация и вход:** Проверьте, что новому пользователю начисляется 1 000 бонусов.
...
4. [ ] **Бустер в полете:** При выборе бустера $\times 2$--$\times 4$ маркер отображается на поле, а при пролете шара срабатывает визуальный скачок и вспышка `BOOSTER_ACTIVATED`.
5. [ ] **Продолжение полета после кэшаута:** После нажатия «Забрать» шар не исчезает, а долетает до точки краха, позволяя игроку видеть весь полет.
6. [ ] **Турнирные очки:** Очки начисляются как при выигрыше (+50 за кэшаут + уровни), так и при крахе (за пройденные уровни).
7. [ ] **SSE турнира:** Турнирная таблица обновляется раз в секунду без лишних сетевых GET-запросов в DevTools вкладке Network.
8. [ ] **Выпадение пазлов:** Проверьте, что кусочки пазла собираются в профиле без дубликатов (Bad Luck Protection).
9. [ ] **Шестиугольник характеристик:** В личном кабинете строится полигон из 6 осей (`GET /api/users/me/radar-stats`).
10. [ ] **Тестовое пополнение:** При нажатии кнопки «Пополнить баланс» вызывается `/api/users/me/top-up` и баланс сразу обновляется в шапке.
