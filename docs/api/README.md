...
├── 04-meta-game.md               # 🧩 Мета-игра: 10 пазлов (бустерная экономика, Pity Timer), 6 рангов, 10 ачивок, радар 6 осей
...
## 🏛 4 главных архитектурных правила
...
3. **Экономика бустеров и фрагментов пазла (Booster Fragment Economy):**  
   Бустеры $\times 2, \times 3, \times 4$ приобретаются игроком за **фрагменты пазла** прямо при старте раунда (сетка по умолчанию: 0, 2, 4, 6 фрагментов). Стартовый баланс каждого игрока — **6 фрагментов**, максимальный лимит — **10 фрагментов**. Фрагменты восполняются через победы и Pity Timer.
4. **Разделение каналов данных:**
...
Для быстрой проверки и демонстрации в базе предзаполнены 2 учетные записи:

| Роль | Логин (username) | Email | Пароль | Начальные ресурсы |
|---|---|---|---|---|
| **ADMIN** | `admin` | `admin@stoloto.ru` | `admin123` | 50 000 бонусов, 5 000 турнирных очков, 6 фрагментов |
| **USER** | `test_user` | `test_user@stoloto.ru` | `test1234` | 10 000 бонусов, ранг `aeronaut`, 6 фрагментов |
