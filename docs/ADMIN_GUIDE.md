...
## 🧭 2. Обзор интерфейса: 3 главных блока

Панель разделена на три логические зоны:
1. **Конфигурация раундов (Hot-Reload)** — живая настройка физики полета, множителей, очков и наград без перезапуска сервера.
2. **House Edge и RTP** — контроль честности, процента возврата игрокам и преимущества казино.
3. **Турнир и выплата наград** — ручное подведение итогов турнира, начисление призов победителям и архив соревнований.

```mermaid
graph LR
    Admin["Администратор"] --> Block1["1. Конфигурация игры<br/>(Скорость, Очки, Бустеры)"]
    Admin --> Block2["2. House Edge & RTP<br/>(Баланс выигрышей 96%)"]
    Admin --> Block3["3. Турнир & Призы<br/>(Force Settle, Топ-3)"]
...
| **Очки за бустер** | `50` | **«Супер-бонус бустера»**. Очки, начисляемые игроку в момент пролета через отметку бустера. | Дополнительный стимул выбирать повышенные ставки с бустером. |
...
### 🚀 Множители бустеров (Tier 1 – Tier 4)

В игре есть 4 варианта ставок. Чем выше ставка игрока — тем мощнее бустер спрятан на высоте:

* **Tier 1 (ставка 10 бонусов):** Множитель `1.0×` — обычный полет без ускорения множителя.
* **Tier 2 (ставка 25 бонусов):** Множитель `2.0×` — при пролете через отметку бустера текущий коэффициент **удваивается**!
* **Tier 3 (ставка 50 бонусов):** Множитель `3.0×` — коэффициент **утраивается**!
* **Tier 4 (ставка 100 бонусов):** Множитель `4.0×` — коэффициент **увеличивается в 4 раза**!
